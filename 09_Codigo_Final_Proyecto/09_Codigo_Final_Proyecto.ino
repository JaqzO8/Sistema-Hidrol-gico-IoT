...
#define USAR_THINGSPEAK 0

const char* WIFI_SSID = "NOMBRE_DE_TU_WIFI";
const char* WIFI_PASS = "CLAVE_DE_TU_WIFI";

unsigned long TS_CHANNEL_ID = 123456;
const char* TS_WRITE_API_KEY = "TU_WRITE_API_KEY";
...
EstadoRiesgo evaluarEstado() {
  ...
  if (nivelCM >= NIVEL_MEDIO_CM || lluviaDigital) {
    return MEDIO;
  }
...
void actualizarBuzzerPorRele() {
  ...
  if (estadoActual == NORMAL) {
    ...
    if ((ahora % 1000) < 600) encenderRele();
    else apagarRele();
...
void mostrarOLED() {
  ...
  display.print(lluviaDigital ? "SI" : "NO");
  display.print(" ");
  display.println(lluviaAnalogica);
...
void enviarThingSpeak() {
  ...
  ThingSpeak.setField(2, lluviaDigital ? 1 : 0);
...
void loop() {
  ...
  if (ahora - ultimoTiempoLectura >= INTERVALO_LECTURA_MS) {
    ...
    Serial.print("Lluvia digital: "); Serial.println(lluviaDigital ? "SI" : "NO");
  ...
  if (ahora - ultimoEnvioThingSpeak >= INTERVALO_THINGSPEAK_MS) {
